...
const NOTICE_CONTENT = {
  auth: {
    ...
    description: "You need to sign in to your profile before enrolling in this course.",
    actionLabel: "Sign In",
  },
  profile: {
...
const EnrollmentAccessNotice = ({ variant, onAction }: EnrollmentAccessNoticeProps) => {
    ...
    <div className="flex flex-row justify-between items-center rounded-[12px] w-[530px] h-[102px] p-[20px] bg-[#F8FAFC] border border-[#E5E7EB]">
      <div className="w-[351px] h-[62px] flex flex-col gap-[8px]">
        <div className="w-full h-[24px] flex flex-row items-center gap-[6px]">
          ...
          <p className="text-[#292929] font-[500] leading-[24px]">{content.title}</p>
        </div>
      ...
      <button
        type="button"
        onClick={onAction}
        className="w-[110px] h-[46px] rounded-[8px] border border-[#B7B3F4] bg-[#EEEDFC] px-[12px] py-[10px] gap-[6px] flex flex-row items-center cursor-pointer"
      >
        <p className="text-[#281ED2] font-[400] text-[14px] leading-[26px]">{content.actionLabel}</p>
        <ARROW_RIGHT aria-hidden className="w-[16px] h-[16px] text-[#281ED2] [&_path]:fill-current" />
      </button>
    </div>
  );
};
